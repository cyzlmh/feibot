...
def test_upsert_exec_job_persists_command_and_working_dir(tmp_path) -> None:
    store_path = tmp_path / "cron" / "jobs.json"
    service = CronService(store_path)

    job, status = service.upsert_job(
        name="crypto-monitor",
        schedule=CronSchedule(kind="cron", expr="0 */4 * * *", tz="Asia/Shanghai"),
        message="",
        payload_kind="exec",
        command="uv run skills/crypto-quote/scripts/price_monitor.py --threshold 5 --structured",
        working_dir="/tmp/demo-workspace",
    )

    assert status == "created"
    assert job.payload.kind == "exec"
    assert job.payload.command is not None
    payload = json.loads(store_path.read_text(encoding="utf-8"))
    persisted = next(item for item in payload["jobs"] if item["id"] == job.id)
    assert persisted["payload"]["kind"] == "exec"
    assert persisted["payload"]["command"] == job.payload.command
    assert persisted["payload"]["workingDir"] == "/tmp/demo-workspace"

    reloaded = CronService(store_path).list_jobs(include_disabled=True)
    restored = next(item for item in reloaded if item.id == job.id)
    assert restored.payload.kind == "exec"
    assert restored.payload.command == job.payload.command
    assert restored.payload.working_dir == "/tmp/demo-workspace"


def test_upsert_exec_job_requires_command(tmp_path) -> None:
    service = CronService(tmp_path / "cron" / "jobs.json")

    with pytest.raises(ValueError, match="exec payload requires command"):
        service.upsert_job(
            name="broken-exec",
            schedule=CronSchedule(kind="every", every_ms=60_000),
            message="",

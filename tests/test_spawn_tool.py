...
@pytest.mark.asyncio
async def test_sp_command_forks_feishu_session(monkeypatch, tmp_path):
    loop = AgentLoop(
        bus=MessageBus(),
    ...
    monkeypatch.setattr(loop, "_open_sp_chat", _fake_open_sp_chat)
...
@pytest.mark.asyncio
async def test_sp_command_rejects_non_feishu(tmp_path):
    loop = AgentLoop(
        bus=MessageBus(),
        provider=_FakeProvider(LLMResponse(content="unused")),
        workspace=tmp_path,
    ...
    resp = await loop._process_message(
        InboundMessage(
            ...
            chat_id="direct",
            content="/sp",
            metadata={"msg_type": "text", "message_id": "msg_sp_cli"},
        )
    )

...
def test_get_history_drops_orphan_tool_results_after_truncation() -> None:
    ...
    # Older normal messages to push the tool-call assistant outside the window.
    for i in range(4):
    ...
    # Tail conversation that should remain.
    session.add_message("user", "new question")
    session.add_message("assistant", "new answer")

    # Window starts at the tool result (assistant tool_calls message is truncated away).

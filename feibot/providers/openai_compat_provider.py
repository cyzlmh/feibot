...
if TYPE_CHECKING:
    from feibot.providers.registry import ProviderSpec
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _resolve_model_name(self, model: str | None) -> str:
        """Resolve request model name from config model and selected provider spec.

        Behavior:
        1) If model uses an explicit provider prefix matching this provider
           (e.g. ``openai/glm-5`` when provider is ``openai``), prune that
           routing prefix before calling upstream.
        2) If spec.strip_model_prefix is enabled, prune to the last segment
           (gateway-specific behavior inherited from nanobot).
        """
        ...
        if "/" in model_name:
            ...
            if prefix.lower().replace("-", "_") == spec.name and rest:
                model_name = rest
    ...
    def _parse(self, response: Any, *, requested_model: str | None = None) -> LLMResponse:
        if isinstance(response, str):
            return LLMResponse(content=response, finish_reason="stop", model=requested_model)
        ...
        if response_map is not None:
                ...
                if content is not None:
                    return LLMResponse(
                        ...
                        provider_payload={
                            ...
                            "response_model": response_map.get("model"),
                            "finish_reason": str(response_map.get("finish_reason") or "stop"),
                        },
                    )
            ...
            provider_payload = {
                ...
                "usage": self._extract_usage(response_map) or None,
            }
            return LLMResponse(
                content=content,
    ...
    @classmethod
    def _parse_chunks(cls, chunks: list[Any], *, requested_model: str | None = None) -> LLMResponse:
    ...
    @staticmethod
    def _handle_error(e: Exception, *, requested_model: str | None = None) -> LLMResponse:
    ...
    async def chat(
        ...
        requested_model = self._resolve_model_name(model)
        ...
        try:
            ...
            return self._handle_error(e, requested_model=requested_model)
    ...
    async def chat_stream(
        ...
        try:
            ...
            return self._parse_chunks(chunks, requested_model=requested_model)
        except Exception as e:
            return self._handle_error(e, requested_model=requested_model)

...
class OpenAICodexProvider(LLMProvider):
    ...
    async def chat(
        ...
        del max_tokens, temperature  # Unused by Codex Responses API currently.
        ...
        try:
            ...
            return LLMResponse(
                content=(
                    ...
                    "Run `feibot provider login openai-codex` first. "
                    f"Details: {e}"
                ),
                finish_reason="error",
            )
...
def _friendly_error(status: int, body: str) -> str:
    if status == 401:
        return "unauthorized (401). Run `feibot provider login openai-codex` and try again."
    if status == 403:

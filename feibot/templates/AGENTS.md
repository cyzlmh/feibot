...
Your private workspace is the current workspace. If `FEIBOT_AGENT_BASE_DIR` is set, it is rooted at `$FEIBOT_AGENT_BASE_DIR/workspace`.

You may also have a shared collaboration root at `$FEIBOT_SHARED_WORKDIR`.
...
If work fits one of those categories and `FEIBOT_SHARED_WORKDIR` is available, place it under the matching directory in `$FEIBOT_SHARED_WORKDIR`.
